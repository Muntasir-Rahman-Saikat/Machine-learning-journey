...
#   @output.capture()  # debug
    ...
    def resize_sq(self, bcid):
        """Resizes the checkbox square, compatible with Matplotlib 3.10+ and older versions."""
        ...
        # Mid versions (3.8–3.9)
        elif hasattr(bcid, "_boxes"):
            rect = list(bcid._boxes.values())[0]

        # New versions (3.10+): no rectangles exposed — find first Rectangle in the axes
        else:
            rects = [a for a in bcid.ax.get_children() if isinstance(a, mpatches.Rectangle)]
            if rects:
                rect = rects[0]

        if rect is None:
            raise AttributeError("Cannot locate checkbox rectangle for this Matplotlib version.")
